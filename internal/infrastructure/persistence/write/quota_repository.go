...
func (r *quotaWriteRepository) Delete(ctx context.Context, id uuid.UUID) error {
	return DBWithContext(ctx, r.db).Delete(&QuotaModel{}, "id = ?", id).Error
}
...
func quotaDomainFromModel(model *QuotaModel) *domainquota.Quota {
	return &domainquota.Quota{
		ID:                model.ID,
		MaxImagesPerMonth: model.MaxImagesPerMonth,
		MaxVideosPerMonth: model.MaxVideosPerMonth,
		MaxFileSizeImage:  model.MaxFileSizeImage,
		MaxFileSizeVideo:  model.MaxFileSizeVideo,
		FullPipeline:      model.FullPipeline,
		HistoryRetention:  time.Duration(model.HistoryRetention),
