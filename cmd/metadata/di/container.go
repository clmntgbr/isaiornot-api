...
import (
	...
	"go-api/internal/infrastructure/metadata"
...
func NewContainer(db *gorm.DB, env *config.Config) *Container {
	topology := rabbitmq.DefaultTopology(
		...
		env.MetadataAnalyzeQueue,
		env.MetadataAnalyzeRoutingKey,
	...
	analyzeMetadataHandler := mediacmd.NewAnalyzeMetadataHandler(
		mediaWriteRepo,
		scanWriteRepo,
		minioStorage,
		metadata.NewAnalyzer(),
		upsertSignalHandler,
		publisher,
	)
